...
test("computes a personal health rank against all month 3 participants without returning their records", async () => {
  const { getPersonalHealthRanking } = await import("../api/_lib/health-ranking.js");
  const employees = [
    employee("winner", "Winner", {
      gender: "female",
      m1: { weight: 55, bmi: 19.03, bodyage: 30, muscle: 28, fat: 20 },
      m3: { weight: 55, bmi: 19.03, bodyage: 30, muscle: 28, fat: 20 },
    }),
    employee("self", "Signed In Person", {
      gender: "female",
      m1: { weight: 70, bmi: 24.22, bodyage: 40, muscle: 20, fat: 35 },
      m3: { weight: 63.5, bmi: 21.97, bodyage: 35, muscle: 20.8, fat: 33 },
    }),
    employee("waiting", "Waiting", {
      gender: "male",
      m1: { weight: 90, bmi: 31.14, bodyage: 45, muscle: 30, fat: 30 },
      m2: { weight: 80, bmi: 27.68, bodyage: 40, muscle: 32, fat: 28 },
    }),
    employee("last", "Last", {
      gender: "male",
      m1: { weight: 75, bmi: 25.95, bodyage: 40, muscle: 33, fat: 20 },
      m3: { weight: 80, bmi: 27.68, bodyage: 42, muscle: 32, fat: 25 },
    }),
  ];
...
test("returns a non-ranked result for an employee without complete month 3 data", async () => {
  const { getPersonalHealthRanking } = await import("../api/_lib/health-ranking.js");
  const employees = [
    employee("self", "Signed In Person", {
      gender: "male",
      m1: { weight: 80, bmi: 27.68, bodyage: 40, muscle: 30, fat: 30 },
      m2: { weight: 75, bmi: 25.95, bodyage: 35, muscle: 31, fat: 28 },
    }),
  ];
...
test("employee API returns only the signed-in row plus its global health ranking", async () => {
  ...
  const signedIn = employee("self", "Signed In Person", {
    gender: "female",
    m1: { weight: 70, bmi: 24.22, bodyage: 40, muscle: 20, fat: 35 },
    m3: { weight: 63.5, bmi: 21.97, bodyage: 35, muscle: 20.8, fat: 33 },
  });
  signedIn.entra_oid = oid;
  const winner = employee("winner", "Winner", {
    gender: "female",
    m1: { weight: 55, bmi: 19.03, bodyage: 30, muscle: 28, fat: 20 },
    m3: { weight: 55, bmi: 19.03, bodyage: 30, muscle: 28, fat: 20 },
  });
  const last = employee("last", "Last", {
    gender: "male",
    m1: { weight: 75, bmi: 25.95, bodyage: 40, muscle: 33, fat: 20 },
    m3: { weight: 80, bmi: 27.68, bodyage: 42, muscle: 32, fat: 25 },
  });
